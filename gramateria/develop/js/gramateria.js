...
class Gramateria {
    ...
    codeImportModal() {
        ...
        exportTxt.innerHTML = '<i class="fa fa-download"></i> บันทึก';
        copyHtml.innerHTML = '<i class="fa fa-copy"></i> Copy HTML';
        copyCss.innerHTML = '<i class="fa fa-copy"></i> Copy CSS';
        fileLoader.innerHTML = '<span class="fa fa-file"></span>  นําเข้าไฟล์ข้อความ';
        ...
        fileLoadInput.onchange = (e) => {
            ...
            if (currentFile === undefined) {
                this.msg.error('กรุณาเลือกไฟล์');
                return;
            }
            const allowFileType = ['gram', 'txt'];
            if (!allowFileType.includes(fType)) {
                this.msg.error('คุณสามารถนำเข้าได้เฉพาะนามสกุล .gram หรือ .txt');
                return;
            }
        ...
        copyHtml.onclick = () => {
            ...
            document.execCommand('copy');
            this.msg.success('คุณได้คัดลอกโค้ด HTML เเล้ว!');
        };
        ...
        copyCss.onclick = () => {
            ...
            document.body.removeChild(dummy);
            document.execCommand('copy');
            this.msg.success('คุณได้คัดลอกโค้ด CSS เเล้ว!');
        };
        ...
        this.editor.Commands.add('html-edit', {
            run: (editor, sender) => {
                ...
                this.modal.setTitle('แก้ไขและนำเข้า');
    ...
    init() {
        ...
        this.editor.on('load', (editor) => {
            ...
            traitsSector.innerHTML = '<div class="gjs-sm-sector no-select">' +
                '<div class="gjs-sm-title"><span class="icon-settings fa fa-cog"></span> ตั้งค่า</div>' +
